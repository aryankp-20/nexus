...
export function initPublicSite() {
  ...
  $('#btn-portal-nav')?.addEventListener('click', () => showRoute('view-login'));
  ...
  $('#btn-explore-bank')?.addEventListener('click', () => {
    document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' });
    toast('Access 190+ balanced assessment questions covering all 10 statistical competencies.', 'sparkles');
  });

  const hamburger = $('#pub-hamburger');
  const navLinks = $('#pub-nav-links') || $('.pub-links');
  if (hamburger && navLinks) {
    hamburger.addEventListener('click', (e) => {
      e.stopPropagation();
      navLinks.classList.toggle('show');
    });
    $all('.pub-links a').forEach(a => a.addEventListener('click', () => navLinks.classList.remove('show')));
  ...
  if (ring) {
    const r = (ring.r && ring.r.baseVal) ? ring.r.baseVal.value : 140;
